...
from utils import get_nparams, get_predictions, calc_edge
...
def calc_acc(args, testloader, model, edge_model, device, disp=0):
    accLoss = 0.0
    imCounter = 0
    ...
    with torch.no_grad():
        ...
        for bt, batchdata in enumerate(tqdm.tqdm(testloader)):
            ...
            # print('latent_pupil_dist : ', latent_pupil_dist_bySample)
        ...
        print('Latent space PUPIL dist. Mean: {}'.format(np.nanmean(dists_pupil_latent)))
        print('Segmentation PUPIL dist. Mean: {}'.format(np.nanmean(dists_pupil_seg)))
        print('Latent space IRIS dist. Mean: {}'.format(np.nanmean(dists_iris_latent)))
        print('Segmentation IRIS dist. Mean: {}'.format(np.nanmean(dists_iris_seg)))
