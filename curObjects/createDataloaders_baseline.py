...
path2data = '../../../Datasets'
path2h5 = os.path.join(path2data, 'TEyeD-h5-Edges')
keepOld = True

DS_sel = pickle.load(open('dataset_selections.pkl', 'rb'))
AllDS = CurLib.readArchives(os.path.join(path2data, 'TEyeD-mat-Edges'))
...
for setSel in list_ds:
    ...
    trainObj = DataLoader_riteyes(dataDiv_obj, path2h5, 0, 'test', False, (480, 640), scale=0.5)
    # Test object
    ...
    path2save = os.path.join(os.getcwd(), 'cond_'+setSel+'.pkl')
    print(path2save)
    print(len(trainObj), len(validObj), len(testObj))
